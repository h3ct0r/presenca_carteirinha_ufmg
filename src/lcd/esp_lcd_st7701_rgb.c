...
#include "lcd/esp_lcd_st7701_interface.h"
typedef struct {
...
static esp_err_t panel_st7701_send_init_cmds(st7701_panel_t* st7701) {
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, ST7701_CMD_CND2BKxSEL, (uint8_t[]){ST7701_CMD_BKxSEL_BYTE0, ST7701_CMD_BKxSEL_BYTE1, ST7701_CMD_BKxSEL_BYTE2, ST7701_CMD_BKxSEL_BYTE3, 0x00}, 5), TAG, "Write cmd failed");
    // Set color format
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]){st7701->madctl_val}, 1), TAG, "Write cmd failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD, (uint8_t[]){st7701->colmod_val}, 1), TAG, "Write cmd failed");
...
static esp_err_t panel_st7701_mirror(esp_lcd_panel_t* panel, bool mirror_x, bool mirror_y) {
    ...
    if (st7701->flags.mirror_by_cmd) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, ST7701_CMD_CND2BKxSEL, (uint8_t[]){
                                                                                     ...
                                                                                     ST7701_CMD_BKxSEL_BYTE2,
                                                                                     ST7701_CMD_BKxSEL_BYTE3,
